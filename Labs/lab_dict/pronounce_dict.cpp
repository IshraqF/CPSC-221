...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    std::transform(word1_copy.begin(), word1_copy.end(), word1_copy.begin(), ::toupper);
    std::transform(word2_copy.begin(), word2_copy.end(), word2_copy.begin(), ::toupper);

    auto lookup_word1 = dict.find(word1_copy);
    auto lookup_word2 = dict.find(word2_copy);
    if (lookup_word1 != dict.end()) {
        if (lookup_word2 != dict.end()) {
            if (lookup_word1->second == lookup_word2->second) {
                return true;
            }
        }
    }
