...
void CommonWords::init_file_word_maps(const vector<string>& filenames)
{
    ...
    for (size_t i = 0; i < filenames.size(); i++) {
        ...
        for (auto word : words) {
            auto lookup = frequency.find(word);
            if (lookup != frequency.end()) {
                lookup->second++;
            } else {
                frequency[word] = 1;
            }
...
void CommonWords::init_common()
{
    ...
    for (size_t i=0; i < file_word_maps.size(); i++) {
        for (auto frequencyMap : file_word_maps[i]) {
            auto lookup = common.find(frequencyMap.first);
            if (lookup != common.end()) {
                lookup->second++;
            } else {
                common[frequencyMap.first] = 1;
            }
        }
    }
}
