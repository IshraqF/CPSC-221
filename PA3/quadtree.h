...
public:
    ...
    int minTolerance(const int leaves, int min, int max) const;
...
private:
    ...
    /* =================== start of private PA3 helper functions ============== */

    /** 
     * Recursive helper function for clear()
     * @param curr the root of the quadtree
     */
    void clearRecursive(Node* & curr);

    /**
     * Recursive helper function for copy()
     * @param curr root of the quadtree to be copied to
     * @param other root of the quadtree to be copied over
    */
    void copyRecursive(Node** curr, const Node* other);

    void renderRecursive(PNG &image, Node* node) const;
