  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      if (distance > 200) {
        ...
        pixel->r = (1- (distance * 0.5)/100) * pixel->r;
        pixel->g = (1- (distance * 0.5)/100) * pixel->g;
        pixel->b = (1- (distance * 0.5)/100) * pixel->b;
      }
...
PNG ubcify(PNG image) {
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
        ...
        RGBAPixel coord_pixel = RGBAPixel(pixel->r, pixel->g, pixel->b);

        double yellow_dist = colordist(coord_pixel, ubc_yellow);
        double blue_dist = colordist(coord_pixel, ubc_blue);

        if (yellow_dist <= blue_dist) {
          pixel->r = 247;
          pixel->g = 184;
          pixel->b = 0;
        }
...
PNG watermark(PNG firstImage, PNG secondImage) {
  ...
  for (unsigned x = 0; x < firstImage.width(); x++) {
    for (unsigned y = 0; y < firstImage.height(); y++) {
        ...
        if (second_pixel->r == 255 && second_pixel->g == 255 && second_pixel->b == 255) {
          if (first_pixel->r <= 215) {
            first_pixel->r += 40;
          }
          else {
            first_pixel->r = 255;
